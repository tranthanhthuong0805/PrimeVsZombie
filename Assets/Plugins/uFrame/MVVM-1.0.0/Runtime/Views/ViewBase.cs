...
namespace uFrame.MVVM.Views
{
    ...
    public abstract partial class ViewBase : MVVMComponent, IUFSerializable, IBindable
    {
        ...
        {
            this.BindProperty(ViewModelObject.VisibleProperty, this.VisibleChanged);
        }
    ...
    public partial class ViewBase
    {
        ...
        {
            string json = ViewModelObject.SerializeToJSON();
            Debug.Log(json);
        }
    }
    ...
    public partial class ViewBase
    {
        ...
        public static void SetActiveChild(Transform behaviour, bool isActive)
        {
            Transform transform1 = null;
            if (behaviour != null)
                transform1 = behaviour.transform;

            for (int i = 0; i < transform1.childCount; ++i)
            {
                Transform trans = transform1.GetChild(i);
                SetActive(trans, isActive);
            }
        }

        public static void SetActive(Transform trans, bool isActive)
        {
            GameObject go = null;
            if (trans != null)
                go = trans.gameObject;
        ...
        public static void SetActive(MonoBehaviour behaviour, bool isActive)
        {
            GameObject go = null;
            if (behaviour != null)
        ...
        public static void SetSiblingIndex(MonoBehaviour behaviour, int index)
        {
            Transform trans = null;
            if (behaviour != null)
                trans = behaviour.transform;
            if (trans != null)
        ...
        public static void SetText(Text txt, object content, string format = "{0}")
        {
            if (txt != null && content != null)
            {
                txt.text = string.Format(format, content);
            }
        }
        
        public static void SetText(Text txt, object[] arg, string format)
        {
            if (txt != null)
            {
                txt.text = string.Format(format, arg);
            }
        }

        public static void SetImage(Image img, Sprite spr)
        {
            if (img != null)
            {
                img.sprite = spr;
        ...
        public static ViewBase InitView(ViewBase viewBase, GameObject prefab, ViewModel viewModel, Vector3 initPosition, Vector3 initScale, Transform parent, bool activeAtFirst = true)
        {
            ViewBase view = null;
            if (prefab != null)
            {
                view = viewBase.InstantiateView(prefab, viewModel);
            }
            else
            {
                view = viewBase.InstantiateView(viewModel);
            }
            ...
            trans.gameObject.SetActive(activeAtFirst);

            return view;
        }

        public static void DestroyViewBase(ref ViewBase viewBase)
        {
            if (viewBase != null)
            {
    ...
    {
        public ViewBase InstantiateView(IScene parentScene, ViewModel model)
        {
            return transform.InstantiateView(parentScene, model);
        }

        public ViewBase InstantiateView(IScene parentScene, ViewModel model, string viewName)
        {
            return transform.InstantiateView(parentScene, viewName, model);
        }
